...
val coreConfigs = List("1", "2", "4", "8", "12")
...
def benchmarkWithCores(cores: String): Map[String, Any] = {
  ...
  val transformationTime = benchmark("Transformation") {
    val data = spark.range(1, 50000000)
    val result = data.filter(col("id") % 2 === 0).count()
  }

  // SQL benchmark
  val sqlTime = benchmark("SQL") {
    val df = spark.range(1, 25000000).withColumn("val", col("id") * 2)
    df.createOrReplaceTempView("t")
    spark.sql("SELECT avg(val) FROM t").collect()
  }

  // Parquet I/O benchmark
  val parquetTime = benchmark("Parquet I/O") {
    val df = spark.range(1, 5000000).withColumn("val", col("id") * 3)
